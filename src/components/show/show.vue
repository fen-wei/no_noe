<template>
    <div class="box">
        <div class="con">
            
           <Swipe></Swipe>
           <div class="qwy_al">
               <ul class="qwy_as">
                   <router-link to="/ke" tag="li">
                       <img src="data:image/png;base64," />
                       <font>特色课</font>
                   </router-link>
                   <router-link to="/oto" tag="li">
                       <img src="data:image/png;base64," />
                       <font>一对一辅导</font>
                   </router-link>
                   <li @click="btn()"> 
                       <img src="data:image/png;base64," />
                       <font>学习日历</font>
                   </li>
               </ul>
           </div>
           <nav>
               <div class="qwy-o">
                  明星讲师
               </div>
               <div class="qwy-p">
                   <div class="qwy-pa" v-for="(item,key) in list">
                        <div class="qwy-ps">
                            <img :src="item.img" alt="">
                        </div>
                        <div class="qwy-pd">
                            <p>
                                {{item.name}}
                                <span>
                                    {{item.nam}}
                                </span>
                            </p>
                            <p>{{item.miaoshu}}</p>
                        </div>
                   </div>
                 <div class="qwy-os">
                  明星讲师
               </div>
               <div class="qwy-p">
                   <div class="qwy-pa" v-for="(item,key) in kan">
                        <div class="qwy-ps">
                            <img :src="item.img" alt="">
                        </div>
                        <div class="qwy-pd">
                            <p>
                                {{item.name}}
                                
                            </p>
                            <p>{{item.miaoshu}}</p>
                        </div>
                   </div>
                   </div>
               </div>
           </nav>
        </div>
        <Footer></Footer>
    </div>
</template>
<script>
import Footer from "@/components/footer";
import Swipe from "./swipe/swipe"
import axios from "axios"
export default {
    name:"show",
    data(){
        return{
            list:[],
            kan:[]
        }
    },
    components: {Footer,Swipe},
    mounted(){
        axios.get("/static/s-shouye.json").then((res)=>{
            console.log(res.data.sss)
            this.list=res.data.sss
        });
        axios.get("/static/s-shouye.json").then((res)=>{
            console.log(res.data.sc)
            this.kan=res.data.sc
        })
    },
    methods:{
        btn(){
            this.$router.push({
                path:"/rl"
            })
        }
    }
}
</script>
<style scoped lang="scss"> 
    html,body,.box{
        width: 100%;
        height: 100%;
        display: flex;
        flex-direction: column;
    }
    .con{
        // height: 8rem;
    
        background: #f0f2f5;
        overflow: auto;
    }
    .qwy_as{
        display: flex;
        padding: 0 0.3rem; 
        justify-content: space-around;
       >li{
           background: #fff;
           width: 2.06rem;
           height: 2.06rem;
           border-radius: 0.3rem; 
           display: flex;
           flex-direction: column;
           align-items: center;
           justify-content: center;
            >img{
            width: 0.42rem;
            height: 0.41rem;
            display: block;
        }
        >font{
            margin-top: 0.28rem; 
            color: #504c3d;
            font-size:  0.22rem; 
        }
       }
    }
    .qwy_al{
        width: 100%;
        height: 2.1rem;
        // background: palevioletred;
        position: relative;
        margin-top: -0.6rem; 
    }
    nav{
        width: 100%;
        // height: 20rem;
        padding: 0.34rem 0.2rem;
        // background: paleturquoise;
        box-sizing: border-box;
        margin-bottom: 0.3rem ; 
    }
    .qwy-o{
        box-sizing: border-box;
        width: 100%;
        height: 0.33rem;
        border-left: 0.06rem solid #e93700;
        font-size: 0.32rem; 
        color: #504c3d;
        padding-left:0.16rem; 
        line-height:  0.33rem;
        margin-left: 0.15rem;
        margin-bottom: 0.4rem; 
        box-sizing: border-box;
    }
    .qwy-ps{
        width: 0.84rem;
        height: 0.84rem;
        border-radius: 50%; 
        >img{
             width: 100%;
             height: 100%;
             border-radius: 50%; 
        }
    }
    .qwy-pa{
        width: 100%;
        background: #fff;
        display: flex;
        border-radius: 0.1rem; 
        padding: 0.3rem 0.36rem;
        margin-top: 0.2rem; 
        box-sizing: border-box;
    }
    .qwy-pd{
       margin-left: 0.3rem; 
       >p:nth-of-type(1){
           color: #5a524b;
           font-size: 0.3rem;

               >span{
                   color: #e8834a;
                   font-size: 0.15rem; 
               }
       }
       >p:nth-of-type(2){
           margin-top: 0.18rem; 
           color: #9d9fac;
           font-size: 0.24rem; 
       }
    }
    .qwy-os{
        box-sizing: border-box;
        width: 100%;
        height: 0.33rem;
        border-left: 0.06rem solid #e93700;
        font-size: 0.32rem; 
        color: #504c3d;
        padding-left:0.16rem; 
        line-height:  0.33rem;
        margin-left: 0.15rem;
        margin-bottom: 0.4rem; 
        margin-top: 0.4rem; 
        box-sizing: border-box;
    }
</style>